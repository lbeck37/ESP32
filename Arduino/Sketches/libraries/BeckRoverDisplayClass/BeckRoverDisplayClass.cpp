// BeckRoverDisplayClass.cpp, 5/22/21a
...
void RoverDisplayClass::DisplayBegin(void) {
  Serial << "RoverDisplayClass::DisplayBegin(): Call GLib.begin()" << endl;
  GLib.begin();
  GLib.setRotation(1);
  return;
}  //DisplayBegin
...
void RoverDisplayClass::DisplayLine(const GFXfont stFont, uint16_t usColor, uint16_t usCursorX, uint16_t usCursorY,
  ...
  Serial << "RoverDisplayClass::DisplayLine(): Color= " << usColor << ", CursorX= " << usCursorX << ", CursorY= " << usCursorY << endl;
  Serial << "RoverDisplayClass::DisplayLine(): ClearWidth= " << usClearWidth << ", ClearHeight= " << usClearHeight << endl;
  Serial << "RoverDisplayClass::DisplayLine():bClearText= " << bClearText << ", Size= " << ucSize << ", Text=" << szText << endl;

  int16_t           sClearXstart    = max(0, ((int16_t)usCursorX - 10));
  int16_t           sClearYstart    = max(0, ((int16_t)usCursorY - 18));
  Serial << "RoverDisplayClass::DisplayLine(): sClearXstart= " << sClearXstart << ", sClearYstart= " << sClearYstart << endl;

  if(bClearText){
    Serial << "RoverDisplayClass::DisplayLine(): Call ClearTextBackground()" << endl;
    ClearTextBackground(sClearXstart, sClearYstart, usClearWidth, usClearHeight);
  }
  Serial << "RoverDisplayClass::DisplayLine(): Call DisplayText()" << endl;
  DisplayText( usCursorX, usCursorY, szText, &stFont, ucSize, usColor);
  Serial << "RoverDisplayClass::DisplayLine(): return" << endl;
  return;
} //DisplayLine
