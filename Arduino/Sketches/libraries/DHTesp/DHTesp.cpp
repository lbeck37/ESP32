const char szDHTespFileName[]  = "DHTesp.cpp";
const char szDHTespFileDate[]  = "5/28/21b";
/******************************************************************
...
{
  Serial << "DHTesp::setup() File: " << szDHTespFileName << ", " << szDHTespFileDate << endl;
  Serial << "DHTesp::setup(): setup(): Begin, pin= " << pin << ", model= " << model << endl;
	DHTesp::pin = pin;
	DHTesp::model = model;
	...
	{
		DHTesp::model = DHT22;
	  Serial << "DHTesp::setup(): setup(): Call readSensor() " << model << endl;
		readSensor();
    Serial << "DHTesp::setup(): setup(): Back from readSensor() " << endl;
...
{
  Serial << "DHTesp::getTemperature(): Call readSensor() " << endl;
	readSensor();
	if (error == ERROR_TIMEOUT)
	{ // Try a second time to read
	  Serial << "DHTesp::getTemperature(): getTemperature(): Call readSensor() again because of ERROR_TIMEOUT" << endl;
		readSensor();
	}
  Serial << "DHTesp::getTemperature(): getTemperature(): Return, temperature= " << temperature << endl;
	return temperature;
} //getTemperature
